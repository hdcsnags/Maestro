...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.phase === "pre_build_complete") {
      ...
      // Build plan is derived deterministically from the lane data already in the DB.
      // An LLM call here adds latency and failure risk for no structural benefit —
      // the lane paths and agent assignments are already authoritative from the
      // architect step. Per-agent instructions are generated by buildDeterministicBuildPlan.
      let buildPlan: BuildPlanPayload = buildDeterministicBuildPlan(projectTitle, architectMd, effectiveBuilders, requestedBuildPrompt);
